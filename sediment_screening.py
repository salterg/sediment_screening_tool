...
#parse paria input distributions for use in HFE probability calculation
...
for j in range(0,len(run_matrix)):
    ...
    if j==0: #initialize
        #commented - version using sediment year
        #MB_annual_allruns=np.zeros((len(run_matrix),1+len(t[M==6]))) 
    ...
    MB_Tmt=-Qs_cms*MD*24*60*60*2650/1000/1000+paria_avg_month[M-1] #MC mass balance (monthly)
    counter=0
    #commented - version using sediment year
    ##may want to skip first and last years since these are incomplete years (sediment year runs july 1st to june 30th)
    # MB_annual=np.zeros(1+len(t[M==6]))
    # t_annual=t.values[(M==6) | (t==t.iloc[-1])]
    # for i in range(0, len(MB_Tmt)):
    #     MB_annual[counter]=MB_annual[counter]+MB_Tmt[i] #find annual mass balance, i.e. mass balance on june 30th
    #     if M[i]==6: #move to next sediment year to start july 1
    #         counter=counter+1
