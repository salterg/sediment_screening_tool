...
all_data=pd.read_csv('time_series_sample_MOEA0070.csv') #crss output policy file
...
for j in range(0,len(run_matrix)):
    ...
    variable=all_data.time_series_slot_name_displayed.iloc[idx]
    ...
    t=all_data.time_series_date.iloc[idx].iloc[idx2]
    Q_month=all_data.time_series_value.iloc[idx].iloc[idx2].to_numpy()*1000000
    Powell_elev=all_data.time_series_value.iloc[idx].iloc[idx3].to_numpy()
    t=pd.to_datetime(t)
    idx_repeat=list(compress(range(len(t)),t==t.iloc[0]))
    if len(idx_repeat)>1:
        t=t[0:idx_repeat[1]]
        Q_month=Q_month[0:idx_repeat[1]]
        Powell_elev=Powell_elev[0:idx_repeat[1]]
